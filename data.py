...
def get_writers_df_real(df_target):
    ...
    # 기사 수 카운트용 컬럼 확인 및 생성
    count_column = None
    if '제목' in df_work.columns:
        count_column = '제목'
    elif '경로' in df_work.columns:
        count_column = '경로'
    else:
        # 임시 컬럼 생성 (기사 수 카운트용)
        df_work['_count_temp'] = 1
        count_column = '_count_temp'
    
    # count_column이 실제로 존재하는지 확인
    if count_column is None or count_column not in df_work.columns:
        # count_column이 없으면 빈 DataFrame 반환
        return pd.DataFrame(), pd.DataFrame()
    ...
    # 임시 컬럼 제거
    if '_count_temp' in df_work.columns:
        df_work = df_work.drop(columns=['_count_temp'])
